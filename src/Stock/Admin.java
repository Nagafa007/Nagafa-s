...
public class Admin extends User {
	...
	// Admin features
	/*
	 * // create a user public void createUser(User user) { if(user != null){ //
	 * user account cannot be null if(!Users.contains(user)) { // check thats its
	 * not already added before Users.add(user);
	 * System.out.println(" User created successfully "); } else{
	...
	public void approveDeposit(RegularUser user, double amount) {
		...
		if (!tradingmanager.isTradingSessionOpen()) {
			...
			peekTransaction.markTransactionApproved();
		...
		user.getTransactionsHistory().add(peekTransaction); // Add transaction history anyways
		user.getPendingTransactions().remove(0); // delete processed transaction
	}
